...
def everyPage(url_root, category, categoryCode, flag):
    ...
    # 读取当前进度
    if saveFlag and os.path.exists('save.sport.csv'):
        load3 = pd.read_csv('save.sport.csv', header=None, encoding='utf-8', sep=' ')
    ...
    for pages in range(beg, 1000):
        ...
        #     return 0
            ...
            save = codecs.open('save.sport.csv', "w", encoding='utf-8')
...
def SearchAmazon():
    # 读取配置文件（类别的网址）
    file = pd.read_csv('sport.csv', header=None, encoding='gbk', sep=',')

    global saveFlag
    num = 0

    # 读取存档文件，判断从那个类别开始继续爬
    if os.path.exists('save.sport.csv'):
        load2 = pd.read_csv('save.sport.csv', header=None, encoding='utf-8', sep=' ')
